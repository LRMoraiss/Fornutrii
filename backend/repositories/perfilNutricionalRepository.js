...
class PerfilNutricionalRepository {
  async create(dados) {
    ...
    const query = `
      INSERT INTO perfil_nutricional (usuario_id, peso, altura, idade, foto, objetivo, restricoes)
  ...
  async findByUsuarioId(usuario_id) {
    const query = `SELECT * FROM perfil_nutricional WHERE usuario_id = $1;`;
    const result = await db.query(query, [usuario_id]);
    return result.rows[0];
  }
}

module.exports = new PerfilNutricionalRepository();
